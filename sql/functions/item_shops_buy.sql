CREATE OR REPLACE FUNCTION item_shops_buy(target_server_id INTEGER, target_item_id INTEGER, target_data INTEGER)
RETURNS TABLE (
...
BEGIN
	...
	RETURN QUERY
		...
		FROM (SELECT * FROM item_last_deals_ids_with_attestation_ids(target_server_id, target_item_id, target_data, 'f', week_ago)) AS deals
		...
		ORDER BY
			reason_id, single_price, history.id DESC;
END;
